...
title: "SCOTUS-Stevens: Decision.fctr classification: caret Random Forest"
author: "bdanalytics"
...
setwd("~/Documents/Work/Courses/MIT/Analytics_Edge_15_071x/Lectures/LCTR4_SCOTUS")
```
...
rownames(glb_script_df) <- seq(1, nrow(glb_script_df))
glb_script_df$elapsed_diff <- sapply(seq(1, nrow(glb_script_df)), 
                                      function(step) ifelse(step <= 1, 0,
        glb_script_df[step, "elapsed"] - glb_script_df[step - 1, "elapsed"]))
print(glb_script_df[order(glb_script_df$elapsed_diff, decreasing=TRUE),])
print(sprintf("Total Elapsed Time: %s secs", 
              format(glb_script_df[nrow(glb_script_df), "elapsed"], big.mark=',')))
tmp_script_df <- subset(glb_script_df, chunk_step_minor == 0)[, 
                                            c("chunk_label", "chunk_step_major")]
names(tmp_script_df)[1] <- "chunk_label_major"
#print(tmp_script_df)
plot_script_df <- merge(glb_script_df, tmp_script_df, all.x=TRUE)
